...
function appBaseUrl(): string {
  return (process.env.PUBLIC_APP_URL ?? 'https://sabotext.com').replace(/\/$/, '')
}
...
async function firstPartyCouponId(stripe: Stripe): Promise<string | null> {
  const id = (process.env.STRIPE_FIRST_PARTY_COUPON ?? 'sabotext_first30').trim()
  if (!id) return null
  try {
...
export async function createPartyCheckoutSession(opts: {
  ...
  { url: string; sessionId: string } | { error: string; stripeCode?: string; stripeType?: string }
> {
  if (!stripeConfigured()) {
        ...
        : [
            {
              ...
              price_data: {
                ...
                product_data: {
                  name: plan === 'week' ? 'Sabotext Party — 7 dagar' : 'Sabotext Party — 24 h',
                  description:
                    plan === 'week'
                      ? 'Obegränsat antal spelare i en vecka — perfekt för återkommande sällskap.'
                      : 'Fler spelare i samma spel — saboterera SMS tillsammans.',
