...
import type { Lang, Player, PublicRoom, Room, RoomStatus, RoundResult, Submission } from './types.js'
...
function clampRounds(count: number) {
  return ALLOWED_ROUNDS.includes(count) ? count : 6
}

export function getAllowedRounds() {
  return [...ALLOWED_ROUNDS]
}
...
export function createRoom(
  ...
  const room: Room = {
    ...
    roundCount: clampRounds(roundCount),
    hostPlays,
    status: 'lobby',
    usedPromptIds: [],
    currentRound: 0,
...
export function joinRoom(
  ...
  | { room: Room; playerId: string }
  | { error: string; code?: 'ROOM_FULL' | 'NOT_FOUND' | 'STARTED' } {
  ...
  if (room.players.length >= MAX_PLAYERS) {
    return {
      error: room.language === 'en' ? `Room is full (max ${MAX_PLAYERS})` : `Rummet är fullt (max ${MAX_PLAYERS})`,
      code: 'ROOM_FULL',
    }
  }
...
export function setRoundCount(code: string, playerId: string, count: number): Room | { error: string } {
  ...
  if (!ALLOWED_ROUNDS.includes(count)) return { error: 'Ogiltigt antal rundor' }
...
function beginRound(room: Room) {
  ...
  // Refresh writer order if players changed
...
function enterVote(room: Room) {
  // Need at least one sabotage to vote on; otherwise skip / reveal original only
  const sabotages = room.submissions.filter((s) => !s.isOriginal)
  if (sabotages.length === 0) {
    // Nobody sabotaged — give writer a pity point and move on
...
export function onPhaseTimeout(room: Room) {
  if (room.status === 'write') {
    // Auto-submit a placeholder if writer timed out with no text
...
export function disconnectSocket(socketId: string) {
  ...
  disconnectTimers.set(
    ...
    setTimeout(() => {
      ...
      // still disconnected after grace — transfer host if needed
...
export function toPublicRoom(room: Room, viewerId: string): PublicRoom {
  ...
  if (room.status === 'vote') {
    // Anonymized sabotages only; shuffle so order doesn't leak author
